...
**Data pipeline:** `registry/datasets.yml` is the declarative source of truth for every dataset (publisher, rights, repo, extraction glob). `npm run generate` walks the sibling repos (`../data-*`) per the registry and writes `data/{id}/{id}-monographs.jsonld`, `data/registry.json`, `data/search-index.json` (bilingual titles), `data/cross-links.json` (counterpart index: `{dataset}/{slug}` → same-substance entries in other pharmacopoeias, matched by normalized title across all label languages) — all committed (vendored). `./data` is canonical for builds; `../open-pharmacopoeia/{data,ontology}` are dev-time fallbacks only. Regenerate + commit after changing sibling data or the registry. Counterpart link categories MUST derive the same way the route generator does (`m.category` else the `@id` path segment) or links 404. Known gap: INN aliases (USP "Acetaminophen" vs "Paracetamol") don't link yet — needs a curated alias table (TODO.deploy/04).
